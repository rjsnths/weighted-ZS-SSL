...
class create_recon(nn.Module):
    ...
    def forward(self, inputs):
        ...
        for blk in range(self.num_block):
            ...
            size = dc_cat_k.size(1)
            center = size / 2
            u, v = np.meshgrid(np.arange(size) - center, np.arange(size) - center)
            
            # weighted with 0.75
            H = np.sqrt(u**2 + v**2)
            H = (H / np.max(H)) * 0.25 + 0.75 
            H = torch.as_tensor(H, device = dc_cat_k.device).float()

            # weighted with 0.5
            # H = u**2 + v**2
            # H = (H / np.max(H)) * 0.5 +0.5 
            # H = torch.as_tensor(H, device = dc_cat_k.device).float()

            
            h_dc_cat_k = dc_cat_k.permute(0,3,1,2) * H

            rg_term_i = self.RegConv_i(dc_cat_i.permute((0,3,1,2))).permute((0,2,3,1))
            rg_term_k = (self.RegConv_k(h_dc_cat_k) / H).permute((0,2,3,1))
